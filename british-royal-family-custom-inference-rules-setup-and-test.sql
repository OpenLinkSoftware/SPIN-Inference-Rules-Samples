...
INSERT
  {
    ...
    <#hasUncle>
        a                rdf:Property ;
        rdfs:label       "hasUncle" ;
        rdfs:comment     """Relationship Type that's derived from a SPARQL-based
                            Custom Inference Rule that associates a Person with
                            a Male Person that's a sibling of one of their Parents.
                         """ ;
    ...
    <#hasAuntie>
        ...
        rdfs:comment     """Relationship Type that's derived from a SPARQL-based
                            Custom Inference Rule that associates a Person with
                            a Female Person that's a sibling of one of their Parents.
                         """ ;
        rdfs:domain      <#RoyalPerson> ;
    ...
    <#hasCousin>
        ...
        rdfs:comment     """Relationship Type that's derived from a SPARQL-based
                            Custom Inference Rule that associates a Person with
                            another Person; that's a child of the sibling of one
                            of their Parents.
                         """ ;
        rdfs:domain      <#RoyalPerson> ;
        rdfs:range       <#FemalePerson> ;
    ...
    <#RoyalPerson>
        ...
        rdfs:comment     """A person that inherits properties of both the
                            foaf:Person and schema:Person classes from their
                            respective vocabularies. The properties of this
                            class are defined using custom inference rules
                            via SPARQL.
                         """ ;
        wdrs:describedby <#> .
    ...
    <#FemalePerson>
        ...
        rdfs:comment     """A female person that inherits properties of the
                            Person class identified by relative URI:
                            <#RoyalPerson>. The properties of this class are
                            defined using custom inference rules via SPARQL.
                         """ ;
        wdrs:describedby <#> .
    ...
    <#MalePerson>
        ...
        rdfs:comment     """A male person that inherits properties of the
                            Person class identified by relative URI:
                            <#RoyalPerson>. The properties of this class are
                            defined using custom inference rules via SPARQL.
                         """ ;
...
SPARQL
DROP MACRO LIBRARY <urn:spin:nanotation:demo:royal:family:lib> ;
...
CREATE SPIN LIBRARY <urn:spin:nanotation:demo:royal:family:lib>
  {
    CLASS <#RoyalPerson>
      {
         comment """Class instance generated from a Macro that customizes
                    relations such AS rel:siblingOf while introducing new
                    relations such AS <#hasAuntie>, <#hasUncle>, and
                    <#hasCousin>.
                 """

         defmacro rel:ancestorOf
           { ?this rel:ancestorOf ?n }
           {{
               { ?this rel:parentOf+ ?n }
               UNION
         ...
         defmacro rel:descendantOf
           { ?this rel:descendantOf ?n }
           {{
               { ?this ^rel:parentOf+ ?n }
           }}

         defmacro rel:sibingOf
           { ?this rel:siblingOf ?n }
           {{
               [] rel:parentOf ?this ,
                               ?n .
...
SELECT                   ?person
       rel:ancestorOf AS ?relation
                         ?descendant
WHERE
  { ?person a              <#RoyalPerson> ;
...
WHERE
  {
    { ?parent a               <#RoyalPerson> ;
              rel:parentOf    ?parentOf
    }
    UNION
    { ?person a               <#RoyalPerson> ;
              <#hasAuntie>    ?hasAuntie
    ...
    UNION
    { ?person a               <#RoyalPerson> ;
              rel:siblingOf   ?hasSibling
    }
    UNION
    { ?person a              <#RoyalPerson> ;
              rel:ancestorOf ?hasDescendant
    }
  } ;
